...
class LocalTaskControlHandler(TaskControlHandler):
    ...
    def update_progress(
        ...
        task_logger.debug(
            f"[Task {task_control.task_definition_name}/{task_control.instance_id}] Progress: {progress}%, Status: {status if status else 'N/A'}"
        )

    def update_status(self, task_control: "TaskControl", status: str) -> None:
        task_logger.debug(
            f"[Task {task_control.task_definition_name}/{task_control.instance_id}] Status updated: {status}"
        )

    def request_stop(self, task_control: "TaskControl") -> None:
        super().request_stop(task_control)  # Sets _should_stop_internal
        task_logger.info(
            f"[Task {task_control.task_definition_name}/{task_control.instance_id}] Stop requested (via LocalHandler)."
        )
...
class PoCMockRemoteTaskControlHandler(TaskControlHandler):
    ...
    def log(
        ...
        print(
            f"[PoCMockRemoteTC][LOG][{level.upper()}] Task {task_control.task_definition_name}/{task_control.instance_id}: {message}",
            flush=True,
        )
        if extra_data:
    ...
    def update_progress(
        ...
        print(
            f"[PoCMockRemoteTC][PROGRESS] Task {task_control.task_definition_name}/{task_control.instance_id}: Progress={progress:.2f}% Status='{status if status else ''}'",
            flush=True,
        )

    def update_status(self, task_control: "TaskControl", status: str) -> None:
        print(
            f"[PoCMockRemoteTC][STATUS] Task {task_control.task_definition_name}/{task_control.instance_id}: Status='{status}'",
            flush=True,
        )
    ...
    def request_stop(self, task_control: "TaskControl") -> None:
        super().request_stop(task_control)  # Sets _should_stop_internal
        print(
            f"[PoCMockRemoteTC][STOP_REQUEST] Task {task_control.task_definition_name}/{task_control.instance_id}: Stop requested.",
            flush=True,
        )
...
class TaskControl:
    ...
    def request_stop(self) -> None:
        ...
        if isinstance(
            self._handler, LocalTaskControlHandler
        ):  # Keep existing log for local
            self._handler.log(self, "Stop requested for instance.", "info")
        elif isinstance(self._handler, PoCMockRemoteTaskControlHandler):
            self._handler.log(
                self,
                "Stop requested for instance (will be checked by task loop).",
                "info",
            )

    def update_progress(
        self, progress_val: float, status_msg: Optional[str] = None
    ) -> None:
...
def create_direct_execution_task_control(task_definition_name: str) -> TaskControl:
    ...
    return TaskControl(
        instance_id=instance_id, task_definition_name=task_definition_name
    )
