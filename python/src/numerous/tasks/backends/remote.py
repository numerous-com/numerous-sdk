...
class RemoteExecutionBackend(ExecutionBackend):
    ...
    def execute(
        ...
        """
        logger.info(
            f"RemoteExecutionBackend.execute called for task (metadata): {target_callable}"
        )
        ...
        if not self.api_url:
            future.set_exception(
                BackendError(
                    "Remote backend not configured with API URL. Cannot execute task."
                )
            )
            return
        ...
        future.set_exception(
            NotImplementedError("Remote task execution is not yet implemented.")
        )
    ...
    def cancel_task_instance(
        ...
        """
        logger.info(
            f"RemoteExecutionBackend.cancel_task_instance called for instance: {instance_id}"
        )

        if not self.api_url:
            logger.error(
                "Remote backend not configured with API URL. Cannot cancel task."
            )
            return False

        logger.warning(
            "RemoteExecutionBackend.cancel_task_instance is not fully implemented."
        )
