...
from ..future import LocalFuture  # Assuming LocalFuture is in ..future
...
class ExecutionBackend(ABC):
    ...
    def execute(
        ...
        target_callable: Callable[
            ..., Any
        ],  # The function to execute (e.g., TaskInstance._task_runner)
    ...
    @abstractmethod
    def cancel_task_instance(
        self, instance_id: str, session_id: Optional[str] = None
    ) -> bool:
...
class LocalExecutionBackend(ExecutionBackend):
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        logger.info(
            f"LocalExecutionBackend initialized with max_workers={max_workers or 'default'}"
        )
    ...
    def execute(
        ...
        if (
            ...
            or not hasattr(task_instance, "id")
            or not hasattr(task_instance, "task_control")
        ):
        ...
        try:
            concurrent_future: ConcurrentFuture = self._executor.submit(
            ...
            def _on_done_callback(cf: ConcurrentFuture):
                try:
                    if (
                        cf.cancelled()
                    ):  # This refers to concurrent_future's cancellation
                    ...
                    logger.error(
                        f"Error in LocalFuture callback for task {instance_id}: {e}",
                        exc_info=True,
                    )
                    if not future.done:
            ...
            logger.error(
                f"Failed to submit task {instance_id} to LocalExecutionBackend: {e}",
                exc_info=True,
            )
            future.set_exception(e)
    ...
    def cancel_task_instance(
        ...
        with self._lock:
            ...
            if task_control:
                logger.info(
                    f"Requesting stop for task instance {instance_id} via TaskControl."
                )
                ...
                if concurrent_ft and not concurrent_ft.done():
                    if concurrent_ft.cancel():
                        logger.info(
                            f"concurrent.futures.Future for task {instance_id} was successfully cancelled (task likely not started)."
                        )
                    else:
                        logger.info(
                            f"concurrent.futures.Future for task {instance_id} could not be cancelled (task likely already running or finished)."
                        )
                return True
            logger.warning(
                f"Task instance {instance_id} not found or no TaskControl available for cancellation."
            )
            return False
