...
from .exceptions import MaxInstancesReachedError, SessionNotFoundError, TaskError
from .future import Future, LocalFuture, TaskStatus  # For local execution
...
class Task(Generic[UserCallable, R]):
    ...
    def __init__(
        self, func: UserCallable, config: TaskConfig, expects_task_control: bool
    ):
    ...
    def _wrap_original_func(self, user_func: UserCallable) -> InternalCallable:
        ...
        def wrapped_func_placeholder(
            task_control_placeholder: TaskControl, *args: Any, **kwargs: Any
        ) -> Any:
...
class TaskInstance(Generic[UserCallable, R]):
    ...
    def __init__(self, task_definition: Task[UserCallable, R], session: "Session"):
        ...
        self.task_control = TaskControl(
            instance_id=self.id,
            task_definition_name=self.task_definition.definition_name,
        )
        self._future: Optional[LocalFuture[R]] = None  # For now, only LocalFuture
        self._status: str = TaskStatus.PENDING  # Use TaskStatus enum
    ...
    def is_active(self) -> bool:
        """
        Check if the task instance is active (starting or running).

        This excludes pending instances for concurrency control purposes.
        """
        if self._future is None:
            # No future yet, check if we're starting
            return self._status == "starting"
        # Have future, check its status
        return self._future.status in (TaskStatus.PENDING, TaskStatus.RUNNING)

    @property
    def is_done(self) -> bool:
        """Check if the task instance is done (completed, failed, or cancelled)."""
        if self._future is None:
            # No future yet, check if we're in a terminal state
            return self._status in (
                TaskStatus.COMPLETED,
                TaskStatus.FAILED,
                TaskStatus.CANCELLED,
            )
        # Have future, use its done property
    ...
    def _task_runner(self, *args: Any, **kwargs: Any) -> R:
        ...
        try:
            if self.task_definition.expects_task_control:
                ...
                return self.task_definition._original_func(
                    self.task_control, *args, **kwargs
                )
    ...
    def start(self, *args: Any, **kwargs: Any) -> Future[R]:
        ...
        # Note: get_running_instances_count should not count this instance since it's still pending
        running_count = self.session.get_running_instances_count(
            self.task_definition_name
        )
        if running_count >= self.task_definition.config.max_parallel:
            self.session.remove_task_instance(self)
        ...
        # Update status to starting after concurrency check passes
        ...
        try:
            ...
            self._status = (
                self._future.status
            )  # Update status from future after backend starts it
        except Exception as e:
            self._future.set_exception(
                e
            )  # Ensure future reflects failure if backend.execute fails
            self._status = self._future.status
            self.session.remove_task_instance(
                self
            )  # Clean up from session if start fails badly
            raise
...
def task(
    ...
    *,
    ...
    if _func is None:  # Decorator called with parentheses: @task(...)
        return decorator
    # Decorator called without parentheses: @task
