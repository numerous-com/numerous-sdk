...
class IdempotentOperations:
    ...
    async def upsert_task_definition(
        self, task_name: str, task_definition: TaskDefinition
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"[IdempotentOps] Upserting task definition: {task_name} v{task_definition.version}"
            )
    ...
    async def upsert_task_instance(
        ...
        try:
            ...
            # For now, simulate the operation
            logger.info(
                f"[IdempotentOps] Upserting task instance: {instance_id} (session: {session_id})"
            )
            ...
            if existing_instance:
                logger.debug(
                    f"[IdempotentOps] Instance {instance_id} already exists, returning existing"
                )
    ...
    async def _get_existing_instance(self, instance_id: str) -> Optional[TaskInstance]:
        ...
        try:
            ...
            logger.debug(
                f"[IdempotentOps] Checking for existing instance: {instance_id}"
            )
            return None
