...
from numerous.tasks.control import TaskControl, TaskControlHandler
from numerous.tasks.versioning import extract_task_definition, get_task_version
...
class RemoteTaskControlHandler(TaskControlHandler):
    ...
    def __init__(self, session_id: Optional[str] = None):
        ...
        self._registered_tasks = (
            set()
        )  # Track registered tasks to avoid duplicate registration
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(
                f"RemoteTaskControlHandler initialized with organization client (session: {self.session_id})"
            )
    ...
    async def ensure_task_registered(self, task_control: TaskControl) -> None:
        ...
        try:
            ...
            result = await self._idempotent_ops.upsert_task_definition(
            ...
            logger.info(
                f"[RemoteTC] Task {task_name} registered with version {version}"
            )
    ...
    async def ensure_instance_registered(self, task_control: TaskControl) -> str:
        ...
        try:
            ...
            instance_id = self._idempotent_ops.generate_instance_id(
                self.session_id, task_control.task_definition_name
            )
            ...
            if task_func:
                ...
                version = (
                    f"local-{hash(task_control.task_definition_name) % 100000000:08x}"
                )
            ...
            logger.info(
                f"[RemoteTC] Instance {instance_id} registered for task {task_control.task_definition_name}"
            )
            return instance_id

        except Exception as e:
            logger.error(
                f"Failed to register instance for task {task_control.task_definition_name}: {e}"
            )
    ...
    def log(
        self, task_control: TaskControl, message: str, level: str, **extra_data: Any
    ) -> None:
        """
        ...
        try:
            ...
            if not hasattr(task_control, "_remote_instance_id"):
                instance_id = self._run_async(
                    self.ensure_instance_registered(task_control)
                )
            ...
            raise RuntimeError(
                f"RemoteTaskControlHandler failed to send log to API: {e}"
            ) from e

    def update_progress(
        self, task_control: TaskControl, progress: float, status: Optional[str]
    ) -> None:
        """
        ...
        try:
            ...
            raise RuntimeError(
                f"RemoteTaskControlHandler failed to send progress update to API: {e}"
            ) from e
    ...
    def update_status(self, task_control: TaskControl, status: str) -> None:
        ...
        try:
            ...
            # For now, log locally with API prefix to simulate API communication
            logger.debug(
                f"[RemoteTC][STATUS] Sending to API - Task {task_control.task_definition_name}/{instance_id}: {status}"
            )

        except Exception as e:
            logger.error(f"Failed to send status update to API: {e}")
            raise RuntimeError(
                f"RemoteTaskControlHandler failed to send status update to API: {e}"
            ) from e
    ...
    def request_stop(self, task_control: TaskControl) -> None:
        ...
        try:
            ...
            # For now, log locally with API prefix to simulate API communication
            logger.info(
                f"[RemoteTC][STOP] Sending to API - Stop requested for task {task_control.task_definition_name}/{task_control.instance_id}"
            )

        except Exception as e:
            logger.error(f"Failed to send stop request to API: {e}")
            raise RuntimeError(
                f"RemoteTaskControlHandler failed to send stop request to API: {e}"
            ) from e
    ...
    def check_stop_requested(self, task_control: TaskControl) -> bool:
        ...
        try:
            ...
            raise RuntimeError(
                f"RemoteTaskControlHandler failed to check stop status with API: {e}"
            ) from e
